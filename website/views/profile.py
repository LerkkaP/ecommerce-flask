...
def show_profile_data(user_id):
    ...
    order_query = db.session.execute(text("SELECT DISTINCT watch_id, brand, model, price, quantity, order_date FROM orders JOIN watches ON orders.watch_id = watches.id WHERE orders.user_id = :user_id ORDER BY order_date"), {"user_id": user_id})
