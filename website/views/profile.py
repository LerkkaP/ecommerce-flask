...
def show_profile_data(user_id):
    ...
    order_query = db.session.execute(text("SELECT DISTINCT watch_id, brand, model, price FROM orders JOIN watches ON orders.watch_id = watches.id WHERE orders.user_id = :user_id"), {"user_id": user_id})
